...
public class Character_controller : MonoBehaviour
{
    ...
    void Update()
    {
        if (click && moves != 0 && VariableStorage.player == "1")
        {
            ...
            if (moves == 0)
            {
                Debug.Log("active" + range.activeSelf);
                range.SetActive(true);
                VariableStorage.range = true;
            }

        }
        if(VariableStorage.range == false)
            range.SetActive(false);
        if (moves == 0 && VariableStorage.soldiers_clicked == 5 && range.activeSelf == false && moving)
        {
            VariableStorage.soldier_clicked = false;
            click = false;
            Debug.Log("heyyyy" + moves);
            moves = 5;
            Thread.Sleep(2000);
    ...
    private void OnMouseDown()
    {
        ...
        {
            if (VariableStorage.soldier_clicked == false && VariableStorage.range == false)
            {
                click = true;
